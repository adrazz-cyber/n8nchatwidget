// SEO it Right Chat Widget - v6.1 (Enhanced Response Handling)
...
(function() {
    ...
    function initWidget() {
        console.log("Initializing SEO it Right Chat Widget v6.1");
        ...
        var config = {
            webhook: {
                ...
                responseKeys: userConfig.webhook && userConfig.webhook.responseKeys ? userConfig.webhook.responseKeys : ['message', 'content', 'text', 'response']
            },
            branding: {
    ...
    function sendMessage(elements, config) {
        ...
        xhr.onload = function() {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                processResponse(elements, config, xhr.responseText);
            } else {
                // Handle HTTP error
    ...
    function processResponse(elements, config, responseText) {
        log('Processing response', responseText);
        
        // First, try to parse as JSON
        try {
            const response = JSON.parse(responseText);
            
            // Check for any of the supported response keys
            for (let i = 0; i < config.webhook.responseKeys.length; i++) {
                const key = config.webhook.responseKeys[i];
                
                if (response.hasOwnProperty(key) && response[key]) {
                    // Found a valid response key
                    addMessage(elements, response[key], 'bot');
                    return;
                }
            }
            
            // Check for nested data structure
            if (response.hasOwnProperty('data') && typeof response.data === 'object' && response.data !== null) {
                for (let i = 0; i < config.webhook.responseKeys.length; i++) {
                    const key = config.webhook.responseKeys[i];
                    
                    if (response.data.hasOwnProperty(key) && response.data[key]) {
                        // Found a valid response key in nested data
                        addMessage(elements, response.data[key], 'bot');
                        return;
                    }
                }
            }
            
            // If we got here, no valid response key was found
            log('Response missing expected fields', response);
            addMessage(elements, 'Sorry, I received an invalid response format. Please contact support.', 'bot');
            
        } catch (e) {
            // Not valid JSON, try using as plain text
            log('Not valid JSON, trying as plain text', e);
            
            if (responseText && typeof responseText === 'string' && responseText.trim()) {
                // Use the plain text response
                addMessage(elements, responseText.trim(), 'bot');
            } else {
                // No usable response
